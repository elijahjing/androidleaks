...
import com.squareup.haha.perflib.HahaSpyYj;
...
import static com.leak.sdk.leaklibrary.heap.HahaHelper.isPrimitiveOrWrapperArray;
import static com.leak.sdk.leaklibrary.heap.HahaHelper.isPrimitiveWrapper;
...
import static com.leak.sdk.leaklibrary.heap.LeakTraceElement.Type.LOCAL;
...
final class ShortestPathFinder {
  ...
  private void enqueueGcRoots(Snapshot snapshot) {
    for (RootObj rootObj : snapshot.getGCRoots()) {
      switch (rootObj.getRootType()) {
        case JAVA_LOCAL:
          Instance thread = HahaSpyYj.allocatingThread(rootObj);
  ...
  private void visitRootObj(LeakNode node) {
    ...
    if (rootObj.getRootType() == RootType.JAVA_LOCAL) {
      Instance holder = HahaSpyYj.allocatingThread(rootObj);
